...
def GetModel(dropoutRate,seed):
    ...
    modelInputPreprocessed = tf.keras.applications.mobilenet.preprocess_input(modelInput)
    model = tf.keras.applications.MobileNetV2(weights='imagenet',include_top=False, pooling='max', input_shape=(imageSize,imageSize,3))
    model.trainable = False
    modelOutput = model(modelInputPreprocessed)
    print("Model output shape is {0}".format(modelOutput.shape))
    
    modelOutputDo = tf.keras.layers.Dropout(dropoutRate,seed= seed+1)(modelOutput) # (None , 1280)
    ...
    return tf.keras.Model(name="BengaliMobileNetV2",inputs=modelInput, outputs=(rootDenseOutput, vowelDenseOutput, consonantDenseOutput, bottleNeckDo)),model
